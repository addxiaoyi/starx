...
StarX 是面向 Velocity、Paper 和 Folia 网络的 Java 21 字节码插件；Paper 26.x 实例按平台要求使用 Java 25。项目只发布一个可部署文件：`starx-universal-0.4.9.jar`。同一个 JAR 放入代理端和每个后端实例，由平台加载器选择对应入口。
...
当前发布产物：`starx-universal-0.4.9.jar`
