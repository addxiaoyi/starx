...
- 大厅、管理员转服和两类队列共享单飞与节点并发限制，修复各自创建协调器造成的限制失效。
- 转服取消、超时与旧连接回调按请求隔离，节点名额只释放一次，补齐异步失败统计和一致的耗时快照。
- 登录次数与容量限制采用原子判定，防止并发提交超额通过。
- 连续队列失败提示按真正发送时间计算冷却；取消或重复转服不再误报子服故障。
- TAB 名称映射校验失败保留最后有效配置，重复错误去重，文件恢复后自动重载。
- 跨服 TAB 停用与刷新互斥，取消延迟任务并隔离旧启用周期的回调，防止清理后重新添加条目。
- 新增跨入口共享、转服竞态、并发限流、提示冷却及映射恢复测试。

## [1.0.18] - 2026-09-25

### Fixed

- 多个 TAB 消费者共享每玩家 250ms 采样限频，异常样本同样限频；仅开启跨服 TAB 时按现有 1 秒巡检持续更新。
- 中断超过 2 秒后恢复采样会重建平滑窗口，避免旧连接质量拖慢新值；有效样本计数排除未知及过期值。
- 修正跨服 TAB 热路径契约测试的范围，保留停用清理所需的玩家查询。
- 增加并发限频、异常值、过期恢复和单独跨服 TAB 采样节奏的回归测试。

## [1.0.17] - 2026-09-25

### Performance

- 内置玩家列表与跨服 TAB 共用同一份 Velocity 真实 Ping 快照，避免两套链路重复采样。
- 跨服 TAB 使用最近 5 次有效样本的中位数和指数平滑值，减少瞬时尖峰导致的列表包抖动。
- 跨服 TAB 构建全网快照时缓存玩家 GameProfile，避免每个查看者重复查询目标玩家。

### Correctness

- 负数和超过 10 秒的异常 Ping 保持未知，不伪造为 `0ms`；跨服 TAB 与 `{starx_ping}` / `%starx_ping%` 语义一致。

### Verification

- Velocity 主代码、测试代码、延迟追踪单元测试和发布元数据校验。
...
- 跨服 TAB 仅在子服/名称变化时重建条目；Ping 变化改为原位更新并设置 5ms 阈值，避免每秒移除重加全网玩家造成额外列表包。
- 跨服 TAB 的事件刷新改为合并调度和单飞执行，登录、断线、转服突发时不再堆积重复刷新任务。
- 后端插件覆盖或清除代理注入的 TAB 条目后，低频巡检会按真实在线列表自动恢复；模块停用后延迟任务不会重新写入。
- 同服原生 TAB 条目改为原位装饰，避免重复 UUID 条目；模块停用时保留同服玩家并恢复简洁名称。
- 无效的子服名称映射文件只对同一版本记录一次警告，修复每秒重复读盘和刷日志。
- 后端心跳始终报告完整的命令邮箱零值指标，修复状态字段在队列清空时消失。
- 队列异步超时契约改为验证统一 `TransferCoordinator`，与当前单飞、限流和超时实现保持一致。

### Added

- 为 `starx.cross-server-tab` 增加显式默认模块开关，配置审计可确认该功能的启用状态。

### Verification

- Velocity 主代码、测试代码和发布元数据校验。

## [1.0.14] - 2026-09-20

### Improved

- `cross-server-tab.yml` 支持热加载，修改子服显示名后无需重启代理。
- 配置文件短暂不可读时保留上一份有效映射，避免在线 TAB 名称突然回退。

## [1.0.13] - 2026-09-20

### Improved

- 新增独立 `cross-server-tab.yml` 子服名称映射文件。
- 跨服 TAB 优先显示映射名称，未配置时回退真实 Velocity 服务器名。

## [1.0.12] - 2026-09-20

### Added

- 新增代理端跨服 TAB 聚合：任意子服的玩家都能看到全网已连接玩家，名称前显示其真实当前子服。

### Performance

- 跨服 TAB 仅在登录、断线、转服或条目实际变化时发送更新；低频巡检用于修正后端 TAB 覆盖，不额外发起网络探测。

## [1.0.11] - 2026-09-20

### Improved

- 玩家已经在大厅时，`/sxhub` 和 `/sxhub status` 会明确提示当前状态，避免重复发起转服。

## [1.0.10] - 2026-09-20

### Improved

- `/sxhub status` 现在显示当前转服请求的实际目标大厅节点，便于玩家确认状态。

## [1.0.9] - 2026-09-20

### Fixed

- `/sxnet diagnose` 的异步后端探测增加连接会话隔离，玩家换服或断线后不再收到旧节点结果。
- 诊断探测异常统一显示为不可达，避免异步异常影响玩家命令体验。
- 增加 `diagnose` 子命令补全入口。
...
- 修复 `/sxhub` 未知参数会被当作无参数转服的问题。

## [1.0.7] - 2026-09-20

### Added

- `/hub` 和 `/lobby` 作为 `/sxhub` 的等价别名。

## [1.0.6] - 2026-09-20

### Fixed

- `/sxhub cancel` 现在取消底层未完成连接，避免取消后仍发生转服。

## [1.0.5] - 2026-09-20

### Added

- `/sxhub cancel` 支持取消未完成大厅转服请求，并保持当前连接。

## [1.0.4] - 2026-09-20

### Added

- `/sxhub status` 显示玩家自身的大厅转服状态，并提供参数补全。
...
- 维护开始和恢复时通知在线玩家，并新增 `/sxmaintain status` 状态查询。

## [1.0.1] - 2026-09-20

### Improved

- 首次登录和注册增加聊天框操作指引，保留直接输入密码和 TOTP 的流程。
- `/sxnet diagnose` 改为分段状态和建议提示，并减少内部实现信息展示。

## [1.0.0] - 2026-09-20

### Added

- 转服协调器增加每个后端节点 32 个并发连接上限，避免连接风暴。
- 队列运行快照增加真实连接耗时 P50/P95/P99 和转服成功率。

### Compatibility

- 压缩能力协商保留原协议回退；双方未协商成功时不改变传输格式。

## [0.9.8] - 2026-09-20

### Improved

- 登录验证重复提交时增加低频处理中提示，避免玩家误以为凭据丢失。

## [0.9.7] - 2026-09-20

### Fixed

- 修复并发转服竞态下将重复请求误报为连接失败的问题。

## [0.9.6] - 2026-09-20

### Improved

- 队列连接失败反馈增加真实位置和预计等待时间，帮助玩家判断排队进度。

## [0.9.5] - 2026-09-20

### Added

- 统一转服指标新增取消计数，区分断线或关闭清理与真实连接失败。

## [0.9.4] - 2026-09-20

### Added

- 统一转服指标新增最近连接耗时 P95，帮助区分排队等待和后端连接变慢。

## [0.9.3] - 2026-09-20

### Added

- 队列运行快照新增统一转服成功、拒绝、失败和重复请求计数。

## [0.9.2] - 2026-09-20

### Added

- 队列运行快照新增真实进行中转服数量，区分排队等待和连接处理状态。

## [0.9.1] - 2026-09-20

### Changed

- 普通队列和智能队列统一使用转服协调器，断线时主动取消未完成连接。

## [0.9.0] - 2026-09-20

### Added

- 大厅和管理员转服接入统一 `TransferCoordinator`，共享单飞、超时、取消和真实连接结果。

## [0.8.6] - 2026-09-20

### Improved

- 队列转服失败时增加低频重试提示，避免玩家误以为排队停止。

## [0.8.5] - 2026-09-20

### Improved

- 登录验证提交后立即显示处理中状态，改善慢速外部验证时的反馈。

## [0.8.4] - 2026-09-20

### Fixed

- 修复玩家快速重复执行 `/sxhub` 时可能并发创建多个大厅连接请求的问题。

## [0.8.3] - 2026-09-20

### Fixed

- `/sxnet send` 防止同一玩家同时存在多个转服请求，避免重复点击造成连接竞争。

## [0.8.2] - 2026-09-20

### Fixed

- 修复普通队列与智能队列可能同时接管同一次满服踢出事件的问题。

### Improved

- 智能队列提示改为中文，并明确 VIP 优先级。

## [0.8.1] - 2026-09-20

### Improved

- `/sxhub` 增加转服中、失败和超时反馈，失败时保持当前连接。
- 满服排队提示改为中文，减少玩家理解成本。
...
- 后端踢出玩家时仅尝试一次同类型健康节点回退，无法选择时才回大厅；认证中的玩家不参与该回退。
- 新增 `{starx_premium_display}` / `%starx_premium_display%`，以“验证中、正版、离线账户、未知”展示账户状态。
- 网络状态 API 增加本地玩家平滑 Ping 的 P95 与样本数量，不产生额外玩家网络请求。

### Changed

- 路由选择可排除刚失败的节点，防止故障节点在回退链路中被立即再次选择。
