...
import io.github.addxiaoyi.starx.limbo.server.LimboSessionHandlerImpl;
...
public class LoginTasksQueue {
  ...
  private void initialize(MinecraftConnection connection) throws Throwable {
    ...
    if (connection.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_20_2) < 0
        || (state != StateRegistry.CONFIG
        && !retainsLimboRegistryUntilConfigAcknowledgement(state, connection.getProtocolVersion()))) {
  ...
  private void connectToServer(Logger logger, ConnectedPlayer player, MinecraftConnection connection) {
    if (connection.getProtocolVersion().compareTo(ProtocolVersion.MINECRAFT_1_20_2) < 0) {
      try {
        ...
        || retainsLimboRegistryUntilConfigAcknowledgement(
        connection.getState(), connection.getProtocolVersion())) {
  ...
  static boolean retainsLimboRegistryUntilConfigAcknowledgement(
      StateRegistry state,
      ProtocolVersion version
  ) {
    Objects.requireNonNull(state, "state");
    Objects.requireNonNull(version, "version");
    return version.compareTo(ProtocolVersion.MINECRAFT_1_20_2) >= 0
        && state.name() == null;
  }
