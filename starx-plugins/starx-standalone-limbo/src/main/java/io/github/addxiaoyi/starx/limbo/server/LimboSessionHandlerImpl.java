...
import io.github.addxiaoyi.starx.limbo.Settings;
import io.github.addxiaoyi.starx.LimboSessionHandler;
import io.github.addxiaoyi.starx.player.LimboPlayer;
import io.github.addxiaoyi.starx.limbo.injection.login.confirmation.LoginConfirmHandler;
...
public class LimboSessionHandlerImpl implements MinecraftSessionHandler {
  ...
  private final KeepAliveResponseGuard keepAliveGuard = new KeepAliveResponseGuard();
  ...
  public boolean handle(KeepAlivePacket packet) {
    MinecraftConnection connection = this.player.getConnection();
    KeepAliveResponseGuard.Decision decision = this.keepAliveGuard.classify(
        this.keepAlivePending,
        this.keepAliveKey,
        packet.getRandomId());
    if (decision == KeepAliveResponseGuard.Decision.IGNORE_STALE) {
      if (Settings.IMP.MAIN.LOGGING_ENABLED) {
        LimboAPI.getLogger().debug(
            "{} sent one stale keepalive while switching into Limbo; ignoring it.", this.player);
      }
      return true;
    }
    if (decision == KeepAliveResponseGuard.Decision.REJECT) {
